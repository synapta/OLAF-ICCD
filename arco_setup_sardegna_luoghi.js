...
const queries       = require('./users/sardegna-luoghi/queries');
const parser        = require('./users/sardegna-luoghi/parser');
const enrichments   = require('./users/sardegna-luoghi/enrichments');
const config        = new Config(JSON.parse(fs.readFileSync(`./app/js/config/sardegna-luoghi.json`)));
...
MongoClient.connect("mongodb://localhost:27017/", (err, client) => {
    ...
    db.collection('sardegna-luoghi').drop();
    nodeRequest(queries.getICCDplaces(), (err, res, body) => {
        ...
        storeAgents(db, 0, () => {
            ...
            db.collection('sardegna-luoghi').find({enriched: false, error: {$ne: true}}).count((err, total) => {
                if(err) throw err;
