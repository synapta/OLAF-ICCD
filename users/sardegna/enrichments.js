...
function storeEnrichment(driver, enrichment) {
    return driver.collection('sardegna').findOneAndUpdate({_id: enrichment.author.uri, enriched: false}, {
        $set: {
...
function getNotEnrichedAgents(driver, limit, callback) {
    driver.collection('sardegna').find({enriched: false, error: {$ne: true}}, {limit: limit}).toArray((err, res) => {
        if(err) throw err;
...
function feedEnrichments(driver, callback, limit = 5) {
    driver.collection('sardegna').find({enriched: false}, {fields: {_id: 1}, limit: limit}).toArray((err, res) => {

        // Generate requests for each enrichment uri
        let requests = res.map(el => nodeRequest('http://localhost:3646/api/v1/sardegna/author/' + encodeURIComponent(el._id) + '/?cache=false'));
...
function getAndlockAgent(driver, user, agent, lock, callback) {
    ...
    if(driver) {
        ...
        driver.collection('sardegna').findOneAndUpdate(
            filter,
...
function deleteValidated(driver, callback) {
    driver.collection('sardegna').deleteMany({validated: true}, (err, res) => {
        if(err) throw err;
        callback();
    })
}
...
function resetLocks(driver, callback) {
    driver.collection('sardegna').updateMany({}, {$set: {lock: null}}, (err, res) => {
        if(err) throw err;
        callback();
    });
}
...
function storeMatching(driver, user, option, agent) {
    ...
    return driver.collection('matches').updateOne(document, {$set: Object.assign(document, {timestamp: new Date()})}, {upsert: true}, (err, res) => {
        if(err) throw err;
        driver.collection('sardegna').updateOne({_id: agent}, {$addToSet: {matchedBy: user.username}});
    });
...
function skipAgent(driver, user, agent) {
    ...
    return driver.collection('skipped').updateOne(document, {$set: Object.assign(document, {timestamp: new Date()})}, {upsert: true}, (err, res) => {
        if(err) throw err;
        driver.collection('sardegna').updateOne({_id: agent}, {$addToSet: {skippedBy: user.username}});
    });
...
function getMatchingToValidate(driver, agent, callback) {
    ...
    driver.collection('sardegna').findOne(filter, (err, enrichment) => {
        if(err) throw err;
...
function validateMatching(driver, agent, callback) {
    ...
    driver.collection('sardegna').findOneAndUpdate({_id: agent}, {$set: {validated: true}}, (err, res) => {
        if(err) throw err;
...
function insertAgents(driver, urisAndUsers, callback) {
    ...
    driver.collection('sardegna').insertMany(documents, (err, res) => {
        if(err) throw err;
        callback();
    })
...
function skipEnrichment(driver, uri) {
    return driver.collection('sardegna').updateOne({_id: uri.id}, {$set: {error: true}});
}
