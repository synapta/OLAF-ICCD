...
function storeEnrichment(driver, enrichment) {
    return driver.collection('sardegna-contenitori').findOneAndUpdate({_id: enrichment.author.uri, enriched: false}, {
        $set: {
...
function getNotEnrichedAgents(driver, limit, callback) {
    driver.collection('sardegna-contenitori').find({enriched: false, error: {$ne: true}}, {limit: limit}).toArray((err, res) => {
        if(err) throw err;
...
function feedEnrichments(driver, callback, limit = 5) {
    driver.collection('sardegna-contenitori').find({enriched: false}, {fields: {_id: 1}, limit: limit}).toArray((err, res) => {

        console.log(err);

        // Generate requests for each enrichment uri
        let requests = res.map(el => nodeRequest('http://localhost:3646/api/v1/sardegna-contenitori/author/' + encodeURIComponent(el._id) + '/?cache=false'));
...
function getAndlockAgent(driver, user, place, lock, callback, count=0) {
    ...
    if( count === 10) {
        ...
        driver.collection('sardegna-contenitori').findOneAndUpdate(
            filter,
...
function deleteValidated(driver, callback) {
    driver.collection('sardegna-contenitori').deleteMany({validated: true}, (err, res) => {
        if(err) throw err;
        callback();
    })
}
...
function resetLocks(driver, callback) {
    driver.collection('sardegna-contenitori').updateMany({}, {$set: {lock: null}}, (err, res) => {
        if(err) throw err;
        callback();
    });
}
...
function storeMatching(driver, user, option, place) {
    ...
    return driver.collection('matches').updateOne(document, {$set: Object.assign(document, {timestamp: new Date()})}, {upsert: true}, (err, res) => {
        if(err) throw err;
        driver.collection('sardegna-contenitori').updateOne({_id: place}, {$addToSet: {matchedBy: user.username}});
    });
...
function skipAgent(driver, user, place) {
    ...
    return driver.collection('skipped').updateOne(document, {$set: Object.assign(document, {timestamp: new Date()})}, {upsert: true}, (err, res) => {
        if(err) throw err;
        driver.collection('sardegna-contenitori').updateOne({_id: place}, {$addToSet: {skippedBy: user.username}});
    });
...
function getMatchingToValidate(driver, place, callback) {
    ...
    driver.collection('sardegna-contenitori').findOne(filter, (err, enrichment) => {
        if(err) throw err;
...
function validateMatching(driver, place, callback) {
    ...
    driver.collection('sardegna-contenitori').findOneAndUpdate({_id: place}, {$set: {validated: true}}, (err, res) => {
        if(err) throw err;
...
function insertThings(driver, urisClassUsers, callback) {
    ...
    driver.collection('sardegna-contenitori').insertMany(documents, (err, res) => {
        if(err) throw err;
        callback();
    })
...
function skipEnrichment(driver, uri) {
    return driver.collection('sardegna-contenitori').updateOne({_id: uri.id}, {$set: {error: true}});
}
