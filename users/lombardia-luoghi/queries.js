...
where {
    {
    graph <http://dati.beniculturali.it/lombardia/architetture> {
            ?site a cis:Site .
        }
    }UNION{
        graph <http://dati.beniculturali.it/lombardia/archeologie> {
            ?site a cis:Site .
        }
    } UNION {
    graph <http://dati.beniculturali.it/lombardia/opere> {
            ?site a cis:Site . 
        }
    }UNION{
        graph <http://dati.beniculturali.it/lombardia/pst>{
            ?site a cis:Site .
        }
    }
    MINUS  {
      ...
      MINUS  {
        graph <http://olaf.datipubblici.org/olaf/sameAs/Lombardia/Sites> {?site a <https://olaf.datipubblici.org/olaf/SkippedEntity>}
      }
      OPTIONAL {
        GRAPH ?g {
...
function storeMatching(item, target) {
    return composeWriteQuery(insertMatchValidation(item, target, 'http://olaf.datipubblici.org/olaf/sameAs/Lombardia/Sites'));
}

function storeSkip(item) {
    return composeWriteQuery(insertSkipValidation(item, 'http://olaf.datipubblici.org/olaf/sameAs/Lombardia/Sites'));
}
