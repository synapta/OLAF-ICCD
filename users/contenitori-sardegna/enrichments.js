...
function storeEnrichment(driver, enrichment) {
    return driver.collection('contenitori-sardegna').findOneAndUpdate({_id: enrichment.author.uri, enriched: false}, {
        $set: {
...
function getNotEnrichedAgents(driver, limit, callback) {
    driver.collection('contenitori-sardegna').find({enriched: false, error: {$ne: true}}, {limit: limit}).toArray((err, res) => {
        if(err) throw err;
...
function feedEnrichments(driver, callback, limit = 5) {
    driver.collection('contenitori-sardegna').find({enriched: false}, {fields: {_id: 1}, limit: limit}).toArray((err, res) => {

        console.log(err);

        // Generate requests for each enrichment uri
        let requests = res.map(el => nodeRequest('http://localhost:3646/api/v1/contenitori-sardegna/author/' + encodeURIComponent(el._id) + '/?cache=false'));
...
function getAndlockAgent(driver, user, thing, lock, callback) {
    ...
    if(driver) {
        ...
        driver.collection('contenitori-sardegna').findOneAndUpdate(
            filter,
...
function deleteValidated(driver, callback) {
    driver.collection('contenitori-sardegna').deleteMany({validated: true}, (err, res) => {
        if(err) throw err;
        callback();
    })
}
...
function resetLocks(driver, callback) {
    driver.collection('contenitori-sardegna').updateMany({}, {$set: {lock: null}}, (err, res) => {
        if(err) throw err;
        callback();
    });
}
...
function storeMatching(driver, user, option, thing) {
    ...
    return driver.collection('matches').updateOne(document, {$set: Object.assign(document, {timestamp: new Date()})}, {upsert: true}, (err, res) => {
        if(err) throw err;
        driver.collection('contenitori-sardegna').updateOne({_id: thing}, {$addToSet: {matchedBy: user.username}});
    });
...
function skipAgent(driver, user, thing) {
    ...
    return driver.collection('skipped').updateOne(document, {$set: Object.assign(document, {timestamp: new Date()})}, {upsert: true}, (err, res) => {
        if(err) throw err;
        driver.collection('contenitori-sardegna').updateOne({_id: thing}, {$addToSet: {skippedBy: user.username}});
    });
...
function getMatchingToValidate(driver, thing, callback) {
    ...
    driver.collection('contenitori-sardegna').findOne(filter, (err, enrichment) => {
        if(err) throw err;
...
function validateMatching(driver, thing, callback) {
    ...
    driver.collection('contenitori-sardegna').findOneAndUpdate({_id: thing}, {$set: {validated: true}}, (err, res) => {
        if(err) throw err;
...
function insertThings(driver, urisClassUsers, callback) {
    ...
    driver.collection('contenitori-sardegna').insertMany(documents, (err, res) => {
        if(err) throw err;
        callback();
    })
...
function skipEnrichment(driver, uri) {
    return driver.collection('contenitori-sardegna').updateOne({_id: uri.id}, {$set: {error: true}});
}
