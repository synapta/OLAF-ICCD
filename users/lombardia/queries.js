...
let getAgents = (index) => {
    ...
    WHERE {
      ...
      MINUS  {
        ?person a <https://olaf.datipubblici.org/olaf/SkippedEntity>
      }
      OPTIONAL {
        GRAPH ?g {
...
function storeMatching(item, target) {
    return composeWriteQuery(insertMatchValidation(item, target, 'http://olaf.datipubblici.org/olaf/sameAs/Lombardia/Agents'));
}

function storeSkip(item) {
    return composeWriteQuery(insertSkipValidation(item, 'http://olaf.datipubblici.org/olaf/sameAs/Lombardia/Agents'));
}
