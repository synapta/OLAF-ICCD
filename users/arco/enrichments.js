...
function getNotEnrichedAgents(driver, limit, callback) {
    driver.collection('agents').find({enriched: false, error: {$ne: true}}, {limit: limit}).toArray((err, res) => {
        if(err) throw err;
...
function deleteValidated(driver, callback) {
    driver.collection('agents').deleteMany({validated: true}, (err, res) => {
        if(err) throw err;
        callback();
    })
}
...
function storeMatching(driver, user, option, agent) {
    ...
    return driver.collection('matches').updateOne(document, {$set: Object.assign(document, {timestamp: new Date()})}, {upsert: true}, (err, res) => {
        if(err) throw err;
        driver.collection('agents').updateOne({_id: agent}, {$addToSet: {matchedBy: user.username}});
    });
...
function skipAgent(driver, user, agent) {
    ...
    return driver.collection('skipped').updateOne(document, {$set: Object.assign(document, {timestamp: new Date()})}, {upsert: true}, (err, res) => {
        if(err) throw err;
        driver.collection('agents').updateOne({_id: agent}, {$addToSet: {skippedBy: user.username}});
    });
...
function getMatchingToValidate(driver, agent, callback) {
    ...
    driver.collection('agents').findOne(filter, (err, enrichment) => {
        if(err) throw err;
...
function insertAgents(driver, urisAndUsers, callback) {
    ...
    driver.collection('agents').insertMany(documents, (err, res) => {
        if(err) throw err;
        callback();
    })
...
function skipEnrichment(driver, uri) {
    return driver.collection('agents').updateOne({_id: uri.id}, {$set: {error: true}});
}
