...
function storeMatching(item, target) {
    return composeWriteQuery(insertMatchValidation(item, target, 'http://olaf.datipubblici.org/olaf/sameAs/Lombardia/CulturalInstitutesOrSites'));
}

function storeSkip(item) {
    return composeWriteQuery(insertSkipValidation(item, 'http://olaf.datipubblici.org/olaf/sameAs/Lombardia/CulturalInstitutesOrSites'));
}
